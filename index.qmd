...
Cross-validation takes basic train-test splitting to the next level by partitioning the data multiple times to perform the analysis and then observing the variance in the unbiased results between partitions (Lei, 2020). It is commonly used for model selection by creating multiple models from the training set and comparing their performance on the testing set. There are many different methods of cross-validation, each claiming to address different problems. A popular method is called Monte Carlo cross-validation (MCCV), where the basic idea is to use random ratios without replacement when splitting your data sets [@Xu2004]. MCCV aims to decrease overfitting by avoiding an unnecessarily large model. An alternative to MCCV is k-fold cross-validation [@wongyeh2020]. K-fold cross-validation splits the data into k subsets, then fits the model on every subset but one. The subset that was used to train a model is used as the testing set, and the process is repeated until every subset is used as the testing subset. Even when researchers aim to create new methods of cross-validation, k-folds are still commonly integrated into the updated strategy, as is demonstrated in the literature re	view below.
...
Filzmoser et al. (2009) [@Filzmoser2009] aimed to test a strategy that would enhance complex linear models from small datasets, as well as estimate prediction errors for new observations. Their focus was repeated double cross-validation (rdCV). This strategy involves splitting the data into training and testing sets in an outer loop for estimating prediction, then an inner loop uses the training set produced in the outer loop to perform cross-validation. This process is repeated many times, producing an increasing amount of predicted outcome values. These values can be used for better model performance and variability estimation. Filzmoser et al. (2009) applied this method to two real chemistry data sets to investigate the effect the number of repetitions and segments has on the stability of rdCV’s result. Greater segmentation in the outer loop requires greater computing time and effort, but more segments also mean more values are calculated. The article claims that four segments in the outer loop and seven segments in the inner loop balance these concerns. Variability was also found to decrease with an increasing number of repetitions, so the recommended number of repetitions is 100, as opposed to 20 or 50. The performance of rdCV using these parameters for model selection was compared to the performance of 7-fold CV and bootstrap, and it was found that their variability was much lower using rdCV. 

Xu et al. (2018) [@Xu2018] propose yet another method of cross-validation called representative
splitting cross-validation (RSCV), with their goal being to ensure the calibration and validation sets are representative and uniformly distributed in the experimental space as possible. They also highlight how this method can be used for latent variable selection. RSCV uses the DUPLEX algorithm to split the data sets, then k-fold cross-validation is applied using the split sets. The DUPLEX algorithm ensures that the farthest objects in the data are uniformly distributed throughout the calibration and validation sets. To test the new method, its performance in estimating partial least squares models is compared to that of traditional cross-validation methods using five different data sets. Some of the traditional methods used were left-one-out cross-validation, 10-fold cross-validation, and Monte Carlo cross-validation with varying amounts of objects left out. Model complexity did not vary significantly across cross-validation methods in all but one data set. RSCV created the most simple model for the one data set that did have significant variances. The recommended number of folds for RSCV was found to be 16.
